...
namespace Fonlow.TraceHub
{
    ...
    internal class PriorityQueueBuffer
    {
        ...
        /// <returns></returns>
        public async Task<QueueStatus> SendAll(Func<IList<TraceMessage>, Task> sendTraceMessagesTask)
        {
            ...
            {
                if (await SendSome(sendTraceMessagesTask) == QueueStatus.Failed)
        ...
        async Task<QueueStatus> SendSome(Func<IList<TraceMessage>, Task> sendTraceMessagesTask)
        {
            TraceMessage tm;
            while ((totalEstimatedSize < Fonlow.TraceHub.Constants.TransportBufferSize) && pendingQueue.TryFirst(out tm))
            {
            ...
            if (sendingBuffer.Count > 0)
            {
                try
                {
                    await sendTraceMessagesTask(sendingBuffer);
                    sendingBuffer.Clear();
                }
